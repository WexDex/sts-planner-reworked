...
function interactiveMotion(interactive: boolean, isSelected: boolean): string {
  if (!interactive) {
    if (isSelected) {
      return "cursor-default ring-[3px] ring-amber-400/90 -translate-y-2 scale-[1.03]";
    }
    return "cursor-default";
  }
  if (isSelected) {
    return "cursor-pointer ring-[3px] ring-amber-400/90 -translate-y-2 scale-[1.03]";
  }
  return "cursor-pointer hover:-translate-y-2 hover:scale-[1.03] hover:ring-2 hover:ring-white/25";
}
...
export function getCardVariantChrome(args: {
  ...
  if (variant === "aurora") {
    return {
      root: `relative overflow-hidden border-2 ${typeStyles.border} bg-gradient-to-b ${typeStyles.gradient} shadow-md transition-transform duration-150 ${motion}`,
      topLine:
        "pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/35 to-transparent",
      innerRim: `pointer-events-none absolute inset-[3px] rounded-[inherit] border ${typeStyles.accentBorder} opacity-35`,
      bottomLine:
  ...
  if (variant === "flat") {
    return {
      root: `relative overflow-hidden border-2 ${typeStyles.border} bg-gradient-to-b ${typeStyles.gradient} shadow-sm transition-transform duration-150 ${motion}`,
  ...
  if (variant === "neon") {
    return {
      root: `relative overflow-hidden border border-white/20 bg-gradient-to-b ${typeStyles.gradient} shadow-md transition-transform duration-150 ${motion}`,
      topLine:
      ...
      descBoxExtra: "",
      typeLabelExtra: "",
      costOrbExtra: "ring-2 ring-white/25",
    };
  }

  if (variant === "paper") {
    return {
      root: `relative overflow-hidden border-2 ${typeStyles.border} bg-gradient-to-b ${typeStyles.gradient} shadow-sm saturate-75 contrast-95 ring-1 ring-amber-950/40 transition-transform duration-150 ${motion}`,
      topLine:
  ...
  /* mono */
  return {
    root: `relative overflow-hidden border-2 border-slate-600/90 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 shadow-sm transition-transform duration-150 ${motion}`,
    topLine:
