        ...
        <div className="relative flex w-full flex-col items-stretch gap-3 px-0 py-0">
          ...
          >
            ...
            <div className="ms-auto flex min-w-0 max-w-full items-center justify-end gap-1.5 pl-2 sm:pl-3 min-[1100px]:pl-4">
              {/* Stance indicator — only when non-neutral */}
              {currentStance !== "neutral" && (() => {
                const stanceCfg = STANCE_CFG.find(s => s.stance === currentStance);
                const stanceColors: Record<string, string> = {
                  wrath: "border-red-500/50 bg-red-950/40 text-red-200",
                  calm: "border-blue-500/50 bg-blue-950/40 text-blue-200",
                  divinity: "border-purple-500/50 bg-purple-950/40 text-purple-200",
                };
                return (
                  <span
                    className={`hidden shrink-0 items-center gap-1 rounded-lg border px-2 py-1 text-[10px] font-bold sm:inline-flex ${stanceColors[currentStance] ?? "border-slate-600/50 bg-slate-800/60 text-slate-300"}`}
                    title={`Stance: ${stanceCfg?.label ?? currentStance}`}
                  >
                    <span aria-hidden>
                      {currentStance === "wrath" ? "🔴" : currentStance === "calm" ? "🔵" : "⭐"}
                    </span>
                    {stanceCfg?.label ?? currentStance}
                  </span>
                );
              })()}
              {/* Orb slot indicators */}
              {(gameState?.orbs ?? []).length > 0 && (
                <span className="hidden shrink-0 items-center gap-0.5 sm:inline-flex" title={`Orbs: ${(gameState?.orbs ?? []).join(", ")}`}>
                  {(gameState?.orbs ?? []).map((orb, i) => {
                    const cfg = ORB_CFG.find(o => o.type === orb);
                    return (
                      <span
                        key={i}
                        className={`inline-flex h-6 w-6 items-center justify-center rounded-md border text-[11px] ${cfg?.bg ?? "bg-slate-800/60"} ${cfg?.border ?? "border-slate-600/50"}`}
                        title={cfg?.label ?? orb}
                        aria-hidden
                      >
                        {cfg?.emoji ?? "?"}
                ...
                <div className="flex min-w-0 flex-wrap items-center justify-between gap-2">
                  {showNoPlannerTurn ? (
                    ...
                    <div className="flex min-w-0 flex-wrap items-center gap-1 tabular-nums">
                      ...
                      {currentStance !== "neutral" && (() => {
                        const scfg = STANCE_CFG.find(s => s.stance === currentStance);
                        const stanceColors: Record<string, string> = {
                          wrath: "border-red-500/50 bg-red-950/40 text-red-200",
                          calm: "border-blue-500/50 bg-blue-950/40 text-blue-200",
                          divinity: "border-purple-500/50 bg-purple-950/40 text-purple-200",
                        };
                        const emoji = currentStance === "wrath" ? "🔴" : currentStance === "calm" ? "🔵" : "⭐";
                        return (
                          <span className={`flex items-center gap-0.5 rounded-md border px-1.5 py-0.5 text-[10px] font-bold ${stanceColors[currentStance] ?? "border-slate-600/50 bg-slate-800/40 text-slate-300"}`}>
                            <span aria-hidden>{emoji}</span>
                            {scfg?.label ?? currentStance}
                          </span>
                        );
                      })()}
                      {(gameState?.orbs ?? []).length > 0 && (
                        <span className="flex items-center gap-0.5">
                          {(gameState?.orbs ?? []).map((orb, i) => {
                            ...
                            return (
                              <span
                                key={i}
                                className={`inline-flex h-5 w-5 items-center justify-center rounded-md border text-[10px] ${cfg?.bg ?? "bg-slate-800/60"} ${cfg?.border ?? "border-slate-600/50"}`}
                                title={cfg?.label ?? orb}
                                aria-hidden
                              >
                                {cfg?.emoji ?? "?"}
