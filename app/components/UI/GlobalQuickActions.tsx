...
// ── Main component ────────────────────────────────────────────────────────────
export default function GlobalQuickActions({ compact = false }: { compact?: boolean }) {
  const {
    gameState,
    updateGameState,
    discardWholeHand,
  ...
  return (
    <div className="w-full">
      ...
      <button
        type="button"
        onClick={() => setOpen(v => !v)}
        className={`inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border-2 px-3 py-2 text-xs font-bold shadow-md ring-1 transition ${
          open
      ...
      {open && (
        <div className="mt-2 grid grid-cols-1 gap-2 rounded-xl border border-indigo-500/20 bg-slate-950/90 p-3 sm:grid-cols-2">
          ...
          <section className="rounded-xl border border-amber-500/25 bg-amber-950/15 p-3">
            <p className="mb-2 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-amber-300/80">
              ...
              Hand Actions
              <span className="ml-auto font-normal normal-case tracking-normal text-amber-400/60">
                {hand.length} cards
              </span>
            </p>
          ...
          <section className="rounded-xl border border-sky-500/25 bg-sky-950/15 p-3">
            <p className="mb-2 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-sky-300/80">
              ...
              Draw Pile
              <span className="ml-auto font-normal normal-case tracking-normal text-sky-400/60">
